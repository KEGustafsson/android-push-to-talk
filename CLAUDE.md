...
- Audio routing: `audio/AudioRoute` owns `AudioManager.mode` for the session and picks the
  ...
  refused switch falls back to the speaker and retries (700 ms, up to 6 times). `onHeadsetChanged`
  fires on any headset/Bluetooth change and `syncMonitor` then restarts the voice monitor when its
  mic (headset vs phone, different gate tuning and proximity arming) no longer matches the route.
