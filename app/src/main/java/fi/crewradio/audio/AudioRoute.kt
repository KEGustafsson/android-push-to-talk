 ...
 */
class AudioRoute(context: Context, private val onStatus: (String) -> Unit) {
    ...
    private fun apply(announce: Boolean) {
        ...
        val label = when {
            ...
            headset.type == AudioDeviceInfo.TYPE_BLUETOOTH_SCO -> "Headset · " + headset.productName.toString().trim().ifEmpty { "Bluetooth" }
            else -> "Wired headset"
        }
